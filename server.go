...
import (
	pb "awesomeProject18/proto/user_service" // замените на правильный путь
	"context"
	"database/sql"
	"errors"
	"fmt"
...
func (s *server) CheckHash(ctx context.Context, request *pb.CheckHashRequest) (*pb.CheckHashResponse, error) {
	var result bool
...
func main() {
	...
	s := grpc.NewServer()
	pb.RegisterHashingServer(s, &server{db: postgres})
